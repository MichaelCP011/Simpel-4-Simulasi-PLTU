...
def simulation_loop():
    ...
    while not thread_stop_event.is_set():
        if control_state["is_auto"]:
            ...
            control_state["steam_valve"] += error * 0.02
            control_state["fuel_feed"] += error * 0.04
            ...
            target_inlet = target_feedwater_pct + (level_error * 0.15)
            inlet_error = target_inlet - control_state["water_inlet"]
            control_state["water_inlet"] += inlet_error * 0.05
            
            target_air_flow_pct = (control_state["fuel_feed"] / config.MAX_COAL_FEED) * 100 
            air_flow_error = target_air_flow_pct - control_state["air_flow"]
            control_state["air_flow"] += air_flow_error * 0.1
            
            for key in ["steam_valve", "water_inlet", "air_flow"]:
                control_state[key] = max(0.0, min(100.0, control_state[key]))
            control_state["fuel_feed"] = max(0.0, min(config.MAX_COAL_FEED, control_state["fuel_feed"]))
