...
# [UPDATE] Nilai Default yang Aman & Stabil
control_state = {
    "fuel_feed": 190.0, 
    "steam_valve": 76.0,
    "water_inlet": 80.0, 
    "air_flow": 86.0,  # [UPDATE] Air flow dinaikkan agar pembakaran 190 t/h sempurna
    "is_auto": False, 
    "target_mw": 300.0
}
...
def simulation_loop():
    ...
    while not thread_stop_event.is_set():
        if control_state["is_auto"]:
            ...
            # [UPDATE AI] Feed-forward Control untuk Water Level
            # 1. Base pump menyeimbangkan uap yang keluar saat ini
            current_steam = plant.valve.steam_flow_kgs
            target_feedwater_pct = (current_steam / config.MAX_FEEDWATER_RATE_KGS) * 100.0
            
            # 2. Tambahan koreksi jika level air meleset dari 72%
            level_error = 72.0 - plant.boiler.water_level.water_level_pct
            
            # 3. Gabungkan perhitungan
            ...
            # [UPDATE AI] Air Flow melacak Fuel Feed secara optimal
